...
def load_data(closing_file, balance_file):
    logging.info(f"Loading closing position report from: {closing_file}")
    logging.info(f"Loading balance by exchange report from: {balance_file}")
    
    if not os.path.exists(closing_file):
        logging.error(f"Closing position report not found: {closing_file}")
        raise FileNotFoundError(f"Closing position report not found: {closing_file}")
    if not os.path.exists(balance_file):
        logging.error(f"Balance by exchange report not found: {balance_file}")
        raise FileNotFoundError(f"Balance by exchange report not found: {balance_file}")
    
    try:
        raw_closing_df = pd.read_csv(closing_file)
        raw_balance_df = pd.read_csv(balance_file)
    ...
    # --- CHANGED: robust two-pass parsing for Date Acquired ---
    # First try day-first; then fill remaining NaT with month-first
...
def reallocate_excess(closing_df, discrepancies, balance_df):
    ...
    for currency in discrepancies['Currency'].unique():
        ...
        # Calculate initial excess per account
        ...
        for _, shortage in cex_shortages.iterrows():
            ...
            available_tax_lots = excess_tax_lots.sort_values(
                ...
                ascending=[False, True, False]  # Highest purchase price, oldest date
            )
            ...
            for idx, tax_lot in available_tax_lots.iterrows():
                ...
                if source_account not in excess_limits or reallocated_amounts[source_account] >= -excess_limits[source_account]:
                    continue  # Skip if no excess or excess limit reached
                ...
                # --- CHANGED: ensure date is carried over explicitly ---
                new_tax_lot['Date Acquired'] = tax_lot['Date Acquired']
                # ------------------------------------------------------
                # Handle cost basis transfer
                ...
                if adjusted_df.loc[idx, 'Amount'] <= 1e-8:
                    ...
                    # Transfer the remaining cost basis if the tax lot is exhausted
        ...
        for _, shortage in wallet_shortages.iterrows():
            ...
            available_tax_lots = excess_tax_lots.sort_values(
                by=['Purchase Price in USD', 'Date Acquired', 'Amount'],
                ascending=[True, False, False]  # Lowest purchase price, newest date
            )
            remaining = shortage_amount
            for idx, tax_lot in available_tax_lots.iterrows():
                if remaining <= 1e-8:
    ...
    # Check for data inconsistencies in adjusted_df
...
def resolve_global_adjustments(adjusted_df, global_discrepancies, balance_df):
    ...
    # Calculate aggregates for both closing and balance DataFrames
    closing_agg = final_df.groupby(['Currency', 'Account'])['Amount'].sum().reset_index()
    balance_agg = balance_df.groupby(['Currency', 'Account'])['Amount'].sum().reset_index()
    
    # Determine which accounts are already balanced for each currency
    ...
    # Accounts where the discrepancy is negligible are considered balanced
    ...
    for _, global_row in global_discrepancies.iterrows():
        ...
        if global_discrepancy > 1e-8:  # Global shortage
            # Identify accounts with shortage post-reallocation
            account_discrepancies = balance_agg.merge(
                closing_agg,
                on=['Currency', 'Account'],
                how='outer',
            ...
            account_discrepancies['Discrepancy'] = account_discrepancies['Amount_balance'] - account_discrepancies['Amount_closing']
            # Exclude accounts that are already balanced for this currency
        ...
        elif global_discrepancy < -1e-8:  # Global excess
            # Identify accounts with excess post-reallocation
            account_discrepancies = balance_agg.merge(
                closing_agg,
                on=['Currency', 'Account'],
                how='outer',
                ...
                for _, account_row in excess_accounts.iterrows():
                    ...
                    for idx, tax_lot in tax_lots.iterrows():
                        ...
                        if final_df.loc[idx, 'Amount'] <= 1e-8:
                            ...
                            # Transfer remaining cost basis if exhausted
    ...
    # Check for data inconsistencies in final_df
...
# ---------- CHANGED: new layout + needs original_df ----------
def generate_cost_basis_summary(original_df, adjusted_df, write_off_details):
    ...
    # Merge all -> exact order requested
    ...
    return df
# ---------- end change ----------
# ---------- CHANGED: Summary tab shows only changed tokens ----------
def save_combined_report(output_path, raw_balance_df, raw_closing_df, discrepancies_simple, global_discrepancies, adjusted_df, cost_basis_summary, write_off_details, reallocation_details, manual_entries, original_df):
    ...
    # Check and correct negative cost basis
    ...
    # Check and correct significant cost basis for zero-amount entries (exhausted tax lots)
    ...
    if not zero_amount_with_cost.empty:
        # Log the issue for transparency
        ...
        # Since cost basis should have been transferred during reallocation, set to 0
    ...
    # Update derived fields for consistency
    ...
    # ==== Build Summary (totals + ONLY changed tokens) ====
    # Headline totals
    ...
    # Distribution by token (changed only)
    ...
    # manual entries amount by token
    ...
    summary_dist = (original_by_token
                    ...
                    .fillna(0))
    # Filter for rows where any of the three columns have changed values
...
def generate_cost_basis_change_analysis(output_path, adjusted_df, final_adjusted_df):
    ...
    # Ensure columns for comparison exist and are numeric
...
def main(closing_file, balance_file, output_path):
    """
    Main execution logic for WBW.py.
    ...
    try:
        ...
        # Validate dates now that all operations are done on adjusted_df
        _validate_all_dates(adjusted_df)
        # CHANGED: pass original_df as well, to build requested cost basis summary
    ...
    # Generate all individual reports regardless of earlier errors
    try:
